...
def load_data():
    try:
        ...
        height = np.where(gender == 'Male',
                          ...
                          np.random.normal(1.62, 0.07, n)).clip(1.45, 1.98)
        bmi_t  = np.random.choice([17, 22, 26.5, 29, 33, 37, 43], n,
                                   p=[0.10, 0.20, 0.18, 0.16, 0.15, 0.12, 0.09])
        weight = (bmi_t * height**2).clip(39, 165)
        bmi    = weight / height**2

        def bmi2lbl(b):
            if b < 18.5: return 'Insufficient_Weight'
            elif b < 25: return 'Normal_Weight'
            elif b < 27.5: return 'Overweight_Level_I'
            elif b < 30:   return 'Overweight_Level_II'
            elif b < 35:   return 'Obesity_Type_I'
            elif b < 40:   return 'Obesity_Type_II'
            else:          return 'Obesity_Type_III'

        return pd.DataFrame({
            'Gender': gender, 'Age': np.random.normal(24,6,n).clip(14,61).astype(float),
            'Height': np.round(height,3), 'Weight': np.round(weight,2),
            'family_history_with_overweight': np.random.choice(['yes','no'],n,p=[0.81,0.19]),
            'FAVC':  np.random.choice(['yes','no'],n,p=[0.88,0.12]),
            'FCVC':  np.random.choice([1,2,3],n,p=[0.10,0.67,0.23]).astype(float),
            'NCP':   np.random.choice([1,2,3,4],n,p=[0.05,0.15,0.66,0.14]).astype(float),
            'CAEC':  np.random.choice(['no','Sometimes','Frequently','Always'],n,p=[0.01,0.73,0.18,0.08]),
            'SMOKE': np.random.choice(['yes','no'],n,p=[0.02,0.98]),
            'CH2O':  np.random.choice([1,2,3],n,p=[0.24,0.61,0.15]).astype(float),
            'SCC':   np.random.choice(['yes','no'],n,p=[0.04,0.96]),
            'FAF':   np.random.choice([0,1,2,3],n,p=[0.25,0.37,0.28,0.10]).astype(float),
            'TUE':   np.random.choice([0,1,2],n,p=[0.37,0.44,0.19]).astype(float),
            'CALC':  np.random.choice(['no','Sometimes','Frequently','Always'],n,p=[0.01,0.77,0.18,0.04]),
            'MTRANS':np.random.choice(['Automobile','Motorbike','Bike','Public_Transportation','Walking'],
                                       n,p=[0.43,0.02,0.03,0.45,0.07]),
            'NObeyesdad': np.array([bmi2lbl(b) for b in bmi])
        })
